...
// lower_bound는 찾으려는 key 값보다 같거나 큰 숫자 가 배열 몇 번째에서 처음 등장 하는지 찾기 위함
```
...
### (2) decltype
```cpp
//decltype은 괄호의 개체의 타입 그대로 추론
int a = 0;
const int b = 0;

decltype(a) c_11 = a; // a와 동일한 int로 추론됨
decltype(b) d_11 = a; // b와 동일한 const int로 추론됨

// decltype은 함수 인자로 사용 가능
...
decltype(auto) d_14 = Func(10, 20); // 추론 규칙은 decltype을 따름
...
int x = 2;
int y = 4; 
auto result = find_if(begin(v), end(v), [=](int i) { return i > x && i <y; }); //[=]은 외부의 모든 변수를 const로 가져옴
```


## 11. 명시적 형변환을 위한 explicit
### (1) 일반적인 explicit 사용법
```cpp
class T {
public:
